...
export class NetjamServer extends EventEmitter {
  ...
  private prepareForRest(provider: ProviderBase) {
    ...
    const remoteMethods: { type: RemoteMethodType; originalName: string; prefix?: string }[] =
    ...
    for (let method of remoteMethods) {
      ...
      const injectedHandlers =
        Reflect.getMetadata(NJ_INJECTED_HANDLERS, provider, originalName) || [];
      this.express[httpReq](
        this.concatRestPrefixes(this.globalPrefix || '', options?.prefix || '/', prefix || '/'),
        [
          ...injectedHandlers,
          async (req, res) => {
    ...
    for (let method of remoteMethods) {
      ...
      switch (type) {
        case RemoteMethodType.MESSAGE:
          ...
          this.socketEventsTree.message[handlerEvent] = async (data: any[], socket: IoSocket) => {
            ...
            const injectedParams =
              Reflect.getMetadata(NJ_INJECTED_PARAMS, provider, originalName) || [];
            _.times(fArgsLen, (i) => {
        ...
        case RemoteMethodType.REMOTE_CALL:
          ...
          this.socketEventsTree.remoteCall[handlerEvent] = async (
            data: any[],
            socket: IoSocket
          ) => {
            const middlewares =
